...
export default function CancelOrderButton({ orderId }: CancelOrderButtonProps) {
  ...
  const handleCancel = async () => {
    if (!confirm('Bu siparişi iptal etmek istediğinize emin misiniz? İptal edildiğinde stoklar otomatik olarak geri eklenecektir.')) {
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        alert('Siparişiniz başarıyla iptal edildi. Stoklar geri eklendi.');
        router.refresh();
      } else {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 border border-red-200">
      <h2 className="text-xl font-semibold text-gray-900 mb-4">Sipariş İptali</h2>
      ...
      <p className="text-sm text-gray-600 mb-4">
        Siparişinizi iptal etmek istediğinize emin misiniz? İptal edildiğinde stoklar otomatik olarak geri eklenecektir.
      </p>
